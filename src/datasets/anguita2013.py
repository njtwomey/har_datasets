...
class anguita2013(Dataset):
    ...
    def build_labels(self, path, *args, **kwargs):
        ...
        for fold in ('train', 'test'):
            fold_labels = load_csv_data(join(path, fold, f'y_{fold}.txt'))
    ...
    def build_folds(self, path, *args, **kwargs):
        fold = []
        fold.extend([-1 for _ in load_csv_data(join(path, 'train', 'y_train.txt')) for _ in range(self.win_len)])
        fold.extend([+1 for _ in load_csv_data(join(path, 'test', 'y_test.txt')) for _ in range(self.win_len)])
    ...
    def build_index(self, path, *args, **kwargs):
        sub = []
        for fold in ('train', 'test'):
            sub.extend(load_csv_data(join(path, fold, f'subject_{fold}.txt')))
    ...
    def build_data(self, path, modality, location, *args, **kwargs):
        ...
        for fold in ('train', 'test'):
            for l, d in zip((x_data, y_data, z_data), ('x', 'y', 'z')):
                ...
                acc = load_csv_data(join(path, fold, 'Inertial Signals', f'{ty}_{modality}_{d}_{fold}.txt'), astype='np')
